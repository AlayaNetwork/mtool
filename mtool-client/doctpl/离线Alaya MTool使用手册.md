...
为了便于节点进行转账，质押，委托以及治理等相关的操作，_CHAIN_NAME_ 提供了 MTool 来辅助用户：
...
```
_COMMAND_NAME_ --version
```

如果执行结果显示`无法将“_COMMAND_NAME_”项识别为 cmdlet、函数、脚本文件或可运行程序的名称。请检查名称的拼写，如果包括路径，请确保路径正确，然后再试一次`，表示没有安装旧版本不需要执行下面操作。

如果执行结果显示版本号，时间戳等信息表示已安装 MTool，如果 MTool 是旧版本，此时需要备份重要信息，然后再手工卸载旧版本，操作步骤：

**step1. 备份目录**

将 `C:\_COMMAND_NAME_\mtool\current\keystore` 下的所有文件到 D 盘或其他非 `C:\_COMMAND_NAME_` 的目录下。安装完新版本之后需要将备份文件拷贝回 `C:\_COMMAND_NAME_\mtool\current\keystore` 目录下。

**step2. 卸载旧版本**

双击 `C:\_COMMAND_NAME_\unins000.exe` 卸载所有旧版本的 MTool 及其他业务工具。

#### 开始安装

**step1. 下载 MTool 安装包**

在在线机器上，复制链接<https://download.alaya.network/alaya/mtool/windows/_MTOOL_VERSION_/_COMMAND_NAME_.exe> 到浏览器下载 MTool 安装包。

**step2. 安装 MTool**

双击 _COMMAND_NAME_.exe 进行安装。默认安装目录为 C:\_COMMAND_NAME_，建议不要更改此安装目录。弹出界面显示**Completing the mtool Setup Wizard**信息表示安装成功，点击**Finish**即可。
...
```bash
wget https://download.alaya.network/alaya/mtool/linux/_MTOOL_VERSION_/_COMMAND_NAME_.zip
```

**step2. 解压 MTool 工具包**

```bash
(if ! command -v unzip;then sudo apt install unzip; fi;) && unzip _COMMAND_NAME_.zip && cd _COMMAND_NAME_
```

**step3. 下载脚本**

> 脚本下载到<font color="red">_COMMAND_NAME_</font> 目录下，否则脚本无法找到新版本 mtool 的路径。
...
```shell
_COMMAND_NAME_ account new staking
```
...
- 执行命令
  ...
  ```shell
  _COMMAND_NAME_ account recover -k staking
  ```
  ...
  ```shell
  _COMMAND_NAME_ account recover -m staking
  ```
...
```shell
_COMMAND_NAME_ create_observewallet --keystore $ALAYA_MTOOLDIR/keystore/staking.json
```
...
```bash
_COMMAND_NAME_ account balance $keystorename --config $ALAYA_MTOOLDIR/validator/validator_config.json
```
...
```bash
_COMMAND_NAME_ account balance -a $address --config $ALAYA_MTOOLDIR/validator/validator_config.json
```
...
```shell
_COMMAND_NAME_ tx transfer --address $ALAYA_MTOOLDIR/keystore/staking_observed.json --amount "1" --recipient $to_address --config $ALAYA_MTOOLDIR/validator/validator_config.json
```
...
```shell
_COMMAND_NAME_ offlinesign --filelocation $ALAYA_MTOOLDIR/transaction_details/transaction_detail_20191108114241.csv
```
...
```shell
_COMMAND_NAME_ send_signedtx --filelocation $ALAYA_MTOOLDIR/transaction_signature/transaction_signature_20191108114625.csv --config $ALAYA_MTOOLDIR/validator/validator_config.json
```
...
```bash
_COMMAND_NAME_ create_restricting --config $ALAYA_MTOOLDIR/validator/validator_config.json --address $ALAYA_MTOOLDIR/keystore/staking_observed.json --file ./restricting_plans.json
```
...
```bash
_COMMAND_NAME_ unstaking --address $ALAYA_MTOOLDIR/keystore/staking_observed.json --config $ALAYA_MTOOLDIR/validator/validator_config.json
```
...
```bash
_COMMAND_NAME_ increasestaking --amount 5000000 --address $ALAYA_MTOOLDIR/keystore/staking_observed.json --config $ALAYA_MTOOLDIR/validator/validator_config.json
```
...
```bash
_COMMAND_NAME_ submit_textproposal --pid_id 100 --address $ALAYA_MTOOLDIR/keystore/staking_observed.json --config $ALAYA_MTOOLDIR/validator/validator_config.json
```
...
```bash
_COMMAND_NAME_ submit_versionproposal --newversion 0.16.0 --end_voting_rounds 345 --pid_id 100 --address $ALAYA_MTOOLDIR/keystore/staking_observed.json --config $ALAYA_MTOOLDIR/validator/validator_config.json
```
...
```bash
_COMMAND_NAME_ submit_cancelproposal --proposalid 0x444c3df404bc1ce4d869166623514b370046cd37cdfa6e932971bc2f98afd1a6 --end_voting_rounds 12 --pid_id 100 --address $ALAYA_MTOOLDIR/keystore/staking_observed.json --config $ALAYA_MTOOLDIR/validator/validator_config.json
```
...
```bash
_COMMAND_NAME_ vote_textproposal --proposalid 0x444c3df404bc1ce4d869166623514b370046cd37cdfa6e932971bc2f98afd1a6 --opinion yes --address $ALAYA_MTOOLDIR/keystore/staking_observed.json --config $ALAYA_MTOOLDIR/validator/validator_config.json
```
...
```bash
_COMMAND_NAME_ vote_versionproposal --proposalid 0x444c3df404bc1ce4d869166623514b370046cd37cdfa6e932971bc2f98afd1a6 --address $ALAYA_MTOOLDIR/keystore/staking_observed.json --config $ALAYA_MTOOLDIR/validator/validator_config.json
```
...
```bash
_COMMAND_NAME_ vote_cancelproposal --proposalid 0x444c3df404bc1ce4d869166623514b370046cd37cdfa6e932971bc2f98afd1a6 --opinion yes --address $ALAYA_MTOOLDIR/keystore/staking_observed.json --config $ALAYA_MTOOLDIR/validator/validator_config.json
```
...
```bash
_COMMAND_NAME_ submit_paramproposal --pid_id 200 --module $module --paramname $paramname --paramvalue $paramvalue --address $ALAYA_MTOOLDIR/keystore/staking_observed.json --config $ALAYA_MTOOLDIR/validator/validator_config.json
```
...
```bash
_COMMAND_NAME_  vote_paramproposal --proposalid 0x444c3df404bc1ce4d869166623514b370046cd37cdfa6e932971bc2f98afd1a6 --opinion yes --address $ALAYA_MTOOLDIR/keystore/staking_observed.json --config $ALAYA_MTOOLDIR/validator/validator_config.json
```
...
```bash
_COMMAND_NAME_ declare_version --address $ALAYA_MTOOLDIR/keystore/staking_observed.json --config $ALAYA_MTOOLDIR/validator/validator_config.json
```
